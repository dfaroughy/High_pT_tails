...
class open_lhco(object):
    ...
    def __init__(self, file, cuts=True, lhco_header=False, remove_endcaps=[]):
        ...
        file_error=NameError('not an lhco file')
        self.cuts=cuts
        self.parameters={}
        ...
        if file_type[-1]=='lhco':
            self.f=open(file,'r')
        elif file_type[-1]=='gz' and file_type[-2]=='lhco':
            self.f=gzip.open(file,'r')
        else:
            raise file_error
        ...
        for l in self.f:
                ...
                if len(line) > 0 and line == ['0','0','0']:
                    break
                elif '= nevents ! Number of unweighted events requested' in l:
                    self.parameters['nevents']=int(line[1])
                elif 'run_tag ! name of the run' in l:
                    self.parameters['run_tag']=line[1]
                elif 'Integrated weight (pb)  :' in l:
                    self.parameters['xsec']=1000.0*float(line[-1])
                elif '#      900000' in l:
                    self.parameters[line[-1]]=float(line[2])
                elif '1.000000e+00 # c' in l:
                    self.parameters[line[-1]]=1.0
...
class spectrum:
    ...
    def plot(self, fig=None, bins=None, xlabel=None, ylabel=None, xlims=None, ylims=None, 
             loglog=False, loglin=False, linlog=False, density=None, cumulative=None, weights=None, histtype='step', 
             rwidth=None, color='k', label=None, stacked=False, lw=1.0, grid=False, subplot=[1,1,1]):

        if fig==None:
            fig=plt.figure()
        ax=fig.add_subplot(subplot[0],subplot[1],subplot[2])
        plot=plt.hist(self.spectrum,bins=bins,density=density, cumulative=cumulative, weights=weights,lw=lw,
                      histtype=histtype, color=color, label=label, stacked=stacked)
        if xlims==None: 
            xlims=[min(self.spectrum),max(self.spectrum)]
        plt.xlim(xlims)
        plt.ylim(ylims)
        if loglog:
            plt.yscale('log')
